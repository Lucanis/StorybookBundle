...
function skipSourceRender(
  context: Parameters<DecoratorFunction<SymfonyRenderer>>[1]
) {
...
export const sourceDecorator: DecoratorFunction<SymfonyRenderer> = (
  storyFn,
  context
) => {
  const story = storyFn();
  ...
  useEffect(() => {
    ...
    if (source) {
      ...
      addons
        .getChannel()
        .emit(SNIPPET_RENDERED, { id, args: unmappedArgs, source: source });
    }
  });
