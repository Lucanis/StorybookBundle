import type { TwigComponentConfiguration } from "./symfony";
...
export class TwigComponentResolver {
  ...
  resolveNameFromFile(file: string) {
    ...
    for (const [namespace, twigDirectories] of Object.entries(
      ...
      const matchingDirectory = twigDirectories.find((dir) =>
    ...
    throw new Error(
      dedent`Unable to determine template name for file "${file}":`
    );
  }
  ...
  resolveFileFromName(name: string) {
    ...
    try {
      ...
      throw new Error(
        dedent`Unable to find template file for component "${name}": ${err}`
      );
    }
  }
